...
| Game Boy | Nintendo classic VC runtime + RomFS injection | Implemented; family/runtime validation present |
...
| Game Gear | Nintendo/Sega `.GG.m` MArchive runtime | PC structural/round-trip validation complete; real-device launch validation remains |
...
Native GBA builds use `agbcia`. Save type is derived from the target ROM rather than copied from the donor. The generated package uses a GBA AGB_FIRM-compatible Title ID in `0004000000F???00`.
...
The generic preset path currently uses a neutral preset for simple ROMs. Because official releases use game-specific preset IDs and those presets may affect cartridge/save behavior, this route remains experimental until real-device launch and SRAM save/relaunch coverage is established.

## Generated Title IDs

RommHeld does not impersonate official retail Title IDs.

The existing deterministic hash remains each game's preferred generated slot, but generated IDs are now recorded in a persistent allocation registry. If another RommHeld game has already claimed that slot, allocation probes forward to a free slot and persists the result. Classic families share one private-looking high normal-application pool; GBA uses the required AGB_FIRM `0004000000F???00` pool.
...
This prevents collisions between RommHeld-managed generated titles. It does not claim to inventory every manually created third-party CIA that may already exist on a console.
...
- family-specific ROM/container rules
