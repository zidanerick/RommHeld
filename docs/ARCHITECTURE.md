...
The current integration stack is intentionally staged:

```text
main
 │
 └── 3DS/application integration (PR #12)
          │
          └── handheld workspace UI (PR #13)
```

The earlier modular GUI and duplicate Send File PRs have been superseded. Test/CI stabilization for the 3DS branch is now folded into the integration stack.

The UI redesign branch is substantially ahead of `main` and contains its own tests, requirements, 3DS transport, target-profile work, and documentation. It remains based on the 3DS integration branch, so integration order matters.
